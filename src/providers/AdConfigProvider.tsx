  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        data.forEach((ad: any) => {
          if (ad.platform === Platform.OS) {
            if (ad.ad_type === "rewarded_video") rewardedVideoId = ad.ad_id;
            if (ad.ad_type === "home_banner") homeBannerId = ad.ad_id;
          }
        });

        setAds({ rewardedVideoId, homeBannerId });
